...
public class adpPasien extends RecyclerView.Adapter<adpPasien.ViewHolder> {
    private Context context;
    private ArrayList<mPasien> list;

    public adpPasien(Context context, ArrayList<mPasien> list) {
    ...
    @NonNull
    @Override
    public adpPasien.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_pasien, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull adpPasien.ViewHolder holder, int position) {
        mPasien model = list.get(position);
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        CardView cardView;
        TextView nik, nama, alamat, tglLahir, jk, email;

        public ViewHolder(@NonNull View itemView) {
            super(itemView);

            cardView = itemView.findViewById(R.id.cardPasien);
            nik = itemView.findViewById(R.id.pasien_nik);
            nama = itemView.findViewById(R.id.pasien_nama);
            alamat = itemView.findViewById(R.id.pasien_alamat);
            tglLahir = itemView.findViewById(R.id.pasien_tgl_lahir);
            jk = itemView.findViewById(R.id.pasien_jk);
        }
    }

    private void tampilDialogPasien(mPasien model, int position) {
        ...
        info.setText(model.getId() + "\n" + model.getNama());

        popup.findViewById(R.id.btnLihatKonfirmasi).setVisibility(View.GONE);

        popup.findViewById(R.id.btnHapusKonfirmasi).setOnClickListener(v -> {
            alertDialog.dismiss();
            konfirmasiHapus(model.getId(), model.getNama(), position);
        });

        popup.findViewById(R.id.btnBatalKonfirmasi).setOnClickListener(v -> alertDialog.dismiss());
    ...
    private void konfirmasiHapus(String id, String nama, int position) {
        ...
        info.setText("Anda yakin ingin menghapus pasien ini?\n" + nama);

        popup.findViewById(R.id.btnYaInfo).setOnClickListener(v -> {
            if (id == null || id.trim().isEmpty()) {
                Toast.makeText(context, "ID pasien kosong", Toast.LENGTH_SHORT).show();
                return;
            }
